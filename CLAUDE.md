...
структурированные блоки `currentCycle`/`lastCycle` (контракт v2, ADR-0011). Control не
детектирует границы — `CycleProcessingHandler` открывает `ImmCycle` (`EndTime = null`)
по `currentCycle`, закрывает (`EndTime`, `DurationSeconds`, `InjectionDurationMs`,
`Cushion`) по совпавшему `lastCycle`. Идентичность цикла — составной ключ
`(CycleNumber, StartTime)`, **не** один номер: счётчик коннектора легитимно
обнуляется между сериями выпуска без разрыва связи, поэтому номер может повторяться
для разных физических циклов. Открытые (`EndTime = null`) циклы уже учитываются в
износе формы (`Cavities`), но исключены из агрегатов длительности/среднего цикла
(`ReportService`) до закрытия. `ImmCycle.PauseDurationMs` в v2 контракте ничем не
заполняется (пауза между циклами вычисляется потребителем данных, не Control) —
поле сохранено в схеме, но пока всегда `null`.
