...
| `BOOTSTRAP_ADMIN_PASSWORD` | пароль администратора (логин — `admin`). Обязательно: ≥8 символов, заглавная + строчная + цифра + спецсимвол (напр. `-_!@`) — иначе админ **не создастся** и войти не выйдет |
