...
set "IPS=%*"
if "%IPS%"=="" (
  echo Usage: 4-make-cert.bat ^<IP1^> [IP2 ...]   ^(напр. 4-make-cert.bat 192.168.0.10 192.168.1.20^)
  pause & exit /b 1
)
set "CAROOT=%~dp0CAROOT"
if not exist "certs" mkdir certs
echo Генерация сертификата для: %IPS% ...
"%~dp0mkcert.exe" -pkcs12 -p12-file certs\pzp.pfx %IPS% 127.0.0.1 localhost
