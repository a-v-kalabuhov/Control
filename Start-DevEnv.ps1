...
$missingContainers = @('wintime-control-postgres', 'wintime-control-mosquitto') |
...
if (-not (Wait-ForService "http://localhost:5000/api/emulator/instances" 30)) {
...
try {
    $presetIds = Invoke-RestMethod "http://localhost:5000/api/presets/list" -ErrorAction Stop

    if ($presetIds -and $presetIds.Count -gt 0) {
        foreach ($immId in $presetIds) {
            try {
                $preset = Invoke-RestMethod "http://localhost:5000/api/presets/$immId" -ErrorAction Stop
                $body = $preset | ConvertTo-Json -Depth 10
                Invoke-RestMethod "http://localhost:5000/api/emulator/instances" `
...
$summary.Add([PSCustomObject]@{ Процесс = 'Emulator'; PID = $emulatorProc.Id; URL = 'http://localhost:5000' })
if ($frontendProc) {
