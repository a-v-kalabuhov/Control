...
/// <summary>
/// Минимальный интерфейс чтения Modbus. Возвращаемые значения уже приведены
/// к host-порядку (реализация снимает big-endian с провода). Только чтение.
/// </summary>
...
/// <summary>Реализация <see cref="IModbusReader"/> поверх NModbus (Modbus TCP).</summary>
public sealed class NModbusReader : IModbusReader
{
    ...
    public NModbusReader(string host, int port, int timeoutMs)
    {
        ...
        _master.Transport.Retries = 0; // повторы делаем на уровне опросчика
    }
