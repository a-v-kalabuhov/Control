...
- `jsonConfig` — вложенный JSON-объект (не экранированная строка), файл читаем глазами.
  Если сохранённый `jsonConfig` не парсится как JSON, экспорт завершается ошибкой
  «Шаблон содержит некорректную JSON-конфигурацию» (файл не создаётся).
...
- `parseTemplateImport(text)` → поля формы
  ...
  3. `formatVersion` — целое ≥ 1 и ≤ 1, иначе «Версия формата N не поддерживается»;
