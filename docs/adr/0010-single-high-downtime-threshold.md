...
- **Известный дефект, зафиксирован задним числом:** учёт выпуска в `CycleProcessingHandler`
  опирался на эвристику «счётчик изменился ИЛИ режим ушёл из `auto`» и структурно терял ровно
  ...
  полуавтомате — на каждой обычной паузе оператора, то есть систематически. Решено в ADR-0012:
  контракт v2 убирает саму эвристику — Control больше не детектирует границы цикла сам, а
  открывает/закрывает `ImmCycle` строго по `currentCycle`/`lastCycle`, которые публикует
  коннектор.
