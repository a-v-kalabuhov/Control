    ...
    ILogger<TemplateCacheStartupService> logger) : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            // TODO : Необходимо выполнить дополнительную проверку. Если есть активные IMM, зависящие от сломанных шаблонов, то нужнно предпринять дополнительные действия.
            // 1. Такие IMM не могут обслужтваться системой, т.к. любое сообщение от них будет приводить к исключению отсутствия шаблона в кеше.
            // 2. Администратору системы надо исправить сломанные шаблоны.
            // 3. Нужно уведомить администратора через события.
            // 4. Нужно помечать такие IMM статусом ошибки конфигурирования.
            // 5. Нужно учесть статус ошибки конфигурирования в эмуляторе - эмуляция таких IMM не должна запускаться.
                logger.LogWarning(
